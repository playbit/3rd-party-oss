...
//config:	It then spawns /system/event/acpi/<ACTION>[/<PARAMETER>] either via
//config:	run-parts (if the resulting path is a directory) or directly as an
//config:	executable.
//config:
...
//usage:     "\n	-f	Run in foreground"
//usage:     "\n	-c DIR	Config directory [/system/event/acpi]"
 ...
 * It then spawns /system/event/acpi/<ACTION>[/<PARAMETER>] either via run-parts
...
int acpid_main(int argc UNUSED_PARAM, char **argv)
{
	...
	int opts;
	struct pollfd *pfd;
	const char *opt_dir = "/system/event/acpi";
